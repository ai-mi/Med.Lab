...
public class AddLaboratoryResultHandler
{
	...
	public async Task Handle(AddLaboratoryResultCommand cmd)
	{
		var aggregate = await LoadAggregate(cmd.PatientId);

		aggregate.AddResult(cmd.PatientId, Guid.NewGuid(), cmd.TestType,
							cmd.Result, cmd.NormalMin, cmd.NormalMax, cmd.Comment);
